...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .sub { color:var(--muted); font-size:13px; line-height:1.6; flex:1; }
...
<div class="wrap">
  ...
  <div class="note">
    ...
    “距52周低点” = (最新价 − 52周最低) / 52周最低；“总市值”单位为亿元。点击模块卡片可一键筛选；点击任意表头可按该列升降序排序；点击右上角「↻ 刷新数据」可即时拉取最新收盘数据（需通过本地服务打开）。<br>
...
async function refresh(){
  ...
  var old=btn.textContent; btn.disabled=true; btn.textContent='刷新中…';
  try{
    var resp=await fetch('/api/refresh?t='+Date.now());
    if(!resp.ok) throw new Error('bad');
    var j=await resp.json();
    DATA=j.rows; STATS=j.stats;
    ...
    document.getElementById('updated').textContent='数据已更新 · '+STATS.now;
  }catch(e){
    document.getElementById('updated').textContent='实时刷新需通过本地服务打开（已尝试重载静态文件）';
    location.reload();
  }finally{
